...
func _physics_process(delta):
	...
	if slicer_mode_enabled and Input.is_action_just_pressed("left_mouse"):
		...
		for body in $Camera3D/Slicer/Area3D.get_overlapping_bodies().duplicate():
			if body is RigidBody3D and body.get_parent() == get_node("../RigidBodys"):
...
func _input(event):
	...
	if event is InputEventKey and event.pressed:
		if event.keycode == KEY_1:
			slicer_mode_enabled = true
			slicer.visible = true  # show slicer
		elif event.keycode == KEY_2:
			slicer_mode_enabled = false
			slicer.visible = false  # hide slicer
